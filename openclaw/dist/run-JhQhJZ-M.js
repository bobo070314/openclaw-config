...
import { s as ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS_ENV } from "./config-env-vars-CwiGXAPn.js";
...
import { c as setConsoleTimestampPrefix, s as setConsoleSubsystemFilter } from "./console-M5qN3yYg.js";
...
import { a as markGatewayRestartTrace, f as startGatewayRestartTrace, o as measureGatewayRestartTrace, r as createGatewayRestartTraceHandoffEnv, t as captureGatewayRestartTraceHandoff } from "./restart-trace-B6aFUfmq.js";
import { r as withProgress } from "./progress-CN3xUiCO.js";
import { n as setGatewayWsLogStyle } from "./ws-logging-86BGsxSJ.js";
import { n as isGatewayRunFutureConfigAllowed, t as enforceGatewayRunFutureConfigGuard } from "./future-config-guard-D3jLPvq2.js";
import { n as acquireGatewayLock, t as GatewayLockError } from "./gateway-lock-BUCsNssB.js";
...
async function resolveGatewayPasswordOption(opts) {
	...
	if (file) {
		const { readSecretFromFile } = await import("./secret-file-CHECEZt2.js");
...
async function maybeLogPendingControlUiBuild(cfg) {
	...
	const { resolveControlUiRootSync } = await import("./control-ui-assets-CNze5Ckn.js");
...
async function resolveGatewayRunShellEnvFallbackPlan(cfg) {
	const { createConfigRuntimeEnv } = await import("./env-vars-B07xCB_f.js");
	const { resolveShellEnvFallbackTimeoutMs, shouldDeferShellEnvFallback, shouldEnableShellEnvFallback } = await import("./shell-env-D9AOJTAH.js");
	...
	const { resolveShellEnvExpectedKeys } = await import("./shell-env-expected-keys-BpFkW4FX.js");
	return {
		enabled: true,
...
async function loadGatewayRunShellEnvFallback(plan) {
	const { loadShellEnvFallback } = await import("./shell-env-D9AOJTAH.js");
...
async function clearGatewayRunShellEnvFallback(values) {
	...
	const { clearShellEnvAppliedKeys } = await import("./shell-env-D9AOJTAH.js");
...
async function maybeWriteGatewayStartupFailureBundle(err) {
	const { writeDiagnosticStabilityBundleForFailureSync } = await import("./diagnostic-stability-bundle-CJvjWs0j.js");
...
async function runGatewayCommand(opts, hooks = {}) {
	...
	const { clearGatewayRunConfigEnvironment } = await import("./pre-bootstrap-CbqVKvpF.js");
	...
	const { startGatewayServer } = await startupTrace.measure("cli.server-import", () => withProgress({
		...
		indeterminate: true
	}, async () => import("./server-BbDqbTge.js")));
	setConsoleTimestampPrefix(true);
	if (devMode) {
		if (opts.reset) {
			const { recheckGatewayRunReset } = await import("./pre-bootstrap-CbqVKvpF.js");
			if (!await recheckGatewayRunReset({
				opts,
				runtime: defaultRuntime
			})) return;
		}
		const { ensureDevGatewayConfig } = await import("./dev-3ELUt6u4.js");
	...
	if (snapshot) {
		const { applyFinalGatewayRunConfigEnv } = await import("./pre-bootstrap-CbqVKvpF.js");
	...
	if (process.env.OPENCLAW_SERVICE_MARKER?.trim()) {
		const { cleanStaleGatewayProcessesSync } = await import("./restart-stale-pids-CRBCTBHd.js");
	...
	if (opts.force) try {
		const { forceFreePortAndWait, waitForPortBindable } = await import("./ports-CP3c6YjQ.js");
	...
	try {
		...
		if (isGatewayLockError(err)) {
			...
			try {
				const { formatPortDiagnostics, inspectPortUsage } = await import("./ports-DreOJ1uv.js");
				const diagnostics = await inspectPortUsage(port);
				if (diagnostics.status === "busy") for (const line of formatPortDiagnostics(diagnostics)) defaultRuntime.error(line);
			} catch {}
			const { maybeExplainGatewayServiceStop } = await import("./shared-VLpmM8Mc.js");
