...
import { F as isOpenClawDeliveryMirrorAssistantMessage } from "./session-accessor-ArmkJJUW.js";
import { a as parseAssistantTextSignature, s as resolveAssistantMessagePhase } from "./chat-message-content-DacpWQXQ.js";
import "./internal-runtime-context-BH_40W4f.js";
import "./stream-message-shared-CdbBqwfX.js";
import { y as stripEnvelopeFromMessages } from "./session-utils.fs-TFao3OTs.js";
...
import { n as HEARTBEAT_PROMPT } from "./heartbeat-CJqWnUZ5.js";
...
function projectEmptyAssistantErrorMessages(messages) {
	...
	const projected = messages.map((message) => {
		...
		if (Array.isArray(message.content) && message.content.some((block) => {
			...
			const type = block.type;
			return type !== "text" && type !== "thinking" && type !== "reasoning" && type !== "redacted_thinking";
		})) {
			changed = true;
		...
		if (typeof sanitized.content === "string") visibleTexts.push(sanitized.content);
			...
			const entry = block;
			if (entry.type === "text" && typeof entry.text === "string") visibleTexts.push(entry.text);
		}
