...
import { c as formatPluginCompatibilityNotice, r as buildPluginCompatibilitySnapshotNotices } from "./status-sUXQ5AzV.js";
import { a as stripPendingPluginInstallRecords, i as hasPendingPluginInstallRecords, n as commitConfigWriteWithPendingPluginInstalls, s as unchangedPendingPluginInstallRecordIds } from "./plugins-install-record-commit-njyBLji9.js";
import { t as resolveSetupSecretInputString } from "./setup.secret-input-KVHCNk6s.js";
import { r as runSetupMigrationImport, t as detectSetupMigrationSources } from "./setup.migration-import-D3ypjyUW.js";
...
function loadAuthChoiceModule() {
	authChoiceModulePromise ??= import("./auth-choice-BbQUoGSi.js");
	return authChoiceModulePromise;
}
function loadConfigLoggingModule() {
	configLoggingModulePromise ??= import("./logging-DGlH-53u.js");
	return configLoggingModulePromise;
}
function loadModelPickerModule() {
	modelPickerModulePromise ??= import("./model-picker-DKII_0dq.js");
	return modelPickerModulePromise;
}
function loadOnboardConfigModule() {
	onboardConfigModulePromise ??= import("./onboard-config-65ds-VIu.js");
...
async function resolveAuthChoiceModelSelectionPolicy(params) {
	...
	const [{ resolveManifestProviderAuthChoice }, { resolvePluginSetupProvider }] = await Promise.all([import("./provider-auth-choices-BHVaoIrr.js"), import("./setup-registry-CZZVv0Fi.js")]);
...
async function runSetupWizard(opts, runtimeInput, prompter) {
	...
	const onboardHelpers = await import("./onboard-helpers-CaUKBdIp.js");
	...
	if (mode === "remote") {
		const { promptRemoteGatewayConfig } = await import("./onboard-remote-DcJtjYFe.js");
	...
	if (authChoiceFromPrompt) {
		...
		({promptAuthChoiceGrouped} = await import("./auth-choice-prompt-DZeecbTg.js"));
	...
	while (true) {
		...
		if (authChoice === "custom-api-key") {
			const { promptCustomApiConfig } = await import("./onboard-custom-Cfd5X55a.js");
	...
	const { configureGatewayForSetup } = await import("./setup.gateway-config-fdwcCZY-.js");
	...
	if (opts.skipChannels ?? opts.skipProviders) await prompter.note(t("wizard.setup.skipChannels"), t("wizard.setup.channelsTitle"));
		...
		const { listChannelPlugins } = await import("./plugins-C1JNe7pM.js");
		const { setupChannels } = await import("./onboard-channels-YofKmZKO.js");
	...
	if (opts.skipSearch) await prompter.note(t("wizard.setup.skipSearch"), t("wizard.setup.searchTitle"));
		...
		const { setupSearch } = await import("./onboard-search-u3JdBCd4.js");
	...
	if (opts.skipSkills) await prompter.note(t("wizard.setup.skipSkills"), t("wizard.setup.skillsTitle"));
		...
		const { setupSkills } = await import("./onboard-skills-CtiLStpS.js");
		nextConfig = await setupSkills(nextConfig, workspaceDir, runtime, prompter);
	}
	if (flow !== "quickstart") {
		const { setupOfficialPluginInstalls } = await import("./setup.official-plugins-CTA5e2WY.js");
		...
		const { setupPluginConfig } = await import("./setup.plugin-config-C2sZ-Ub0.js");
	...
	if (!opts.skipHooks) {
		const { setupInternalHooks } = await import("./onboard-hooks-BxY7XatC.js");
	...
	nextConfig = await writeSetupConfigFile(nextConfig, { allowConfigSizeDrop: configResetPerformed });
	const { finalizeSetupWizard } = await import("./setup.finalize-C7eQ6WGl.js");
