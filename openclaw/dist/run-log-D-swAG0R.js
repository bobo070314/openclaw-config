...
import { g as getNodeSqliteKysely, h as executeSqliteQueryTakeFirstSync, i as openOpenClawStateDatabase, m as executeSqliteQuerySync, o as runOpenClawStateWriteTransaction } from "./openclaw-state-db-CTUHxhwv.js";
import { t as parseByteSize } from "./parse-bytes-BOhZJPCN.js";
import { c as cronStoreKey } from "./normalize-C1o3G6t5.js";
import { u as resolveFailoverReasonFromError } from "./failover-error-3to6iVLf.js";
import { i as normalizeCronRunDiagnostics } from "./run-diagnostics-scmbjH32.js";
...
async function appendCronRunLog(params) {
	...
	const next = (writesByTarget.get(writeKey) ?? Promise.resolve()).catch(() => void 0).then(async () => {
		runOpenClawStateWriteTransaction(({ db }) => {
			...
			if (params.opts?.keepLines !== false) pruneCronRunLogRows(db, storeKey, params.entry.jobId, params.opts?.keepLines ?? DEFAULT_CRON_RUN_LOG_KEEP_LINES);
		});
	});
