...
import { i as buildAgentMainSessionKey, u as normalizeAgentId } from "./session-key-BsbwBJwv.js";
...
const loadOverviewModule = async () => await import("./overview-CzsazBEt.js");
const loadModelsSharedModule = async () => await import("./shared-DJCcVqh2.js");
const loadConfigCliModule = async () => await import("./config-cli-C90tN-xd.js");
const loadDoctorModule = async () => await import("./doctor-BlWPTKct.js");
...
async function runGatewayLifecycle(operation) {
	const lifecycle = await import("./lifecycle-REDDLlKf.js");
	if (operation === "start") {
...
async function executeCrestodianOperation(operation, runtime, opts = {}) {
	...
	if (operation.kind === "plugin-search") {
		...
		await (opts.deps?.runPluginsSearch ?? (async (query, pluginRuntime) => {
			const { runPluginsSearchCommand } = await import("./plugins-search-command-SL6-0dc0.js");
	...
	if (operation.kind === "plugin-uninstall") {
		...
		await (opts.deps?.runPluginUninstall ?? (async (pluginId, pluginRuntime) => {
			const { runPluginUninstallCommand } = await import("./plugins-uninstall-command-6mSPUbTf.js");
	...
	if (operation.kind === "create-agent") {
		...
		const before = await readConfigFileSnapshot();
		const workspace = resolveUserPath(operation.workspace ?? process.cwd());
		await (opts.deps?.runAgentsAdd ?? (await import("./agents.commands.add-9bq6ZKr2.js")).agentsAddCommand)({
	...
	if (operation.kind === "status") {
		...
		const { statusCommand } = await import("./status.command-B7SPHmqr.js");
	...
	if (operation.kind === "open-tui") {
		...
		const result = await (opts.deps?.runTui ?? (await import("./tui-B9xuiPCJ.js")).runTui)({
			local: true,
			session,
			deliver: false,
			historyLimit: 200
