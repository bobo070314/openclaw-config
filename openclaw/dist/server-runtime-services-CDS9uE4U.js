import { r as isVitestRuntimeEnv } from "./env-DpEiW8Oq.js";
import { t as isGatewayModelPricingEnabled } from "./model-pricing-config-pKJPI_G6.js";
import { r as startHeartbeatRunner } from "./heartbeat-runner-Df4cCdpO.js";
import { r as createNoopHeartbeatRunner } from "./server-runtime-startup-services-BQWgFw3S.js";
...
function recoverPendingOutboundDeliveries(params) {
	(async () => {
		const { recoverPendingDeliveries } = await import("./delivery-queue-B6vA-Qkb.js");
		const { deliverOutboundPayloadsInternal } = await import("./deliver-Dg0UmeCm.js");
...
function recoverPendingSessionDeliveries(params) {
	setTimeout(() => {
		(async () => {
			const { recoverPendingRestartContinuationDeliveries } = await import("./server-restart-sentinel-CS0Rc-4S.js");
...
function startGatewayModelPricingRefreshOnDemand(params) {
	...
	let stopRefresh;
	(async () => {
		const { startGatewayModelPricingRefresh } = await import("./model-pricing-cache-C0jylX0z.js");
		if (stopped) return;
