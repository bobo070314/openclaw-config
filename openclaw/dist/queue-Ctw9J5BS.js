...
import { _ as persistSessionTranscriptTurn, f as loadSessionEntry } from "./session-accessor-ArmkJJUW.js";
...
import { r as clearCommandLane } from "./command-queue-Cn3ZuyZZ.js";
...
import { t as isRoutableChannel } from "./route-reply-BvqNvbYv.js";
...
function createUserTurnTranscriptRecorder(params) {
	...
	const handlePersistenceError = (error) => {
		...
		import("./globals-tmdhXg2m.js").then(({ logVerbose }) => {
...
function resolveOriginRoutingMetadata(items) {
	const source = items.find((item) => item.originatingChannel && item.originatingTo) ?? items.find((item) => item.originatingChannel || item.originatingTo || item.originatingAccountId || item.originatingThreadId != null || item.originatingReplyToId || item.originatingReplyToMode || item.originatingChatType);
	if (!source) return {};
	return {
...
function resolveCrossChannelKey(item) {
	...
	if (!channel && !to && !accountId && (threadId == null || threadId === "") && !replyToId) return chatType ? { key: JSON.stringify(["unresolved", chatType]) } : {};
