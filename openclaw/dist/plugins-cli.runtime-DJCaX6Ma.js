...
import { g as shortenHomeInString } from "./utils-BmpsSbZo.js";
...
import { a as formatMissingPluginMessage } from "./error-format-DbPRORpk.js";
import { n as collectConfiguredRuntimePluginIds, r as resolveConfiguredRuntimePluginInstallCandidate } from "./configured-runtime-plugin-installs-CCIhLG-6.js";
//#region src/cli/plugins-cli.runtime.ts
function createModuleLoader(load) {
	let promise;
	return () => promise ??= load();
}
const loadPluginsConfigState = createModuleLoader(() => import("./config-state-BzyvL-P3.js"));
const loadPluginsStatus = createModuleLoader(() => import("./status-DjyjEUWD.js"));
const loadPluginsCommandHelpers = createModuleLoader(() => import("./plugins-command-helpers-BdiUX1sy.js"));
const loadPluginsRegistryRefresh = createModuleLoader(() => import("./plugins-registry-refresh-CDLv4T_I.js"));
...
async function runPluginsEnableCommand(idInput) {
	...
	const { enableExplicitlySelectedPluginInConfig } = await import("./enable-B_VKbnnx.js");
...
async function runPluginsDisableCommand(idInput) {
	...
	const { normalizePluginId } = await loadPluginsConfigState();
	const { buildPluginRegistrySnapshotReport } = await loadPluginsStatus();
	const { setPluginEnabledInConfig } = await import("./plugins-config-CGtrgNm-.js");
	const { refreshPluginRegistryAfterConfigMutation } = await loadPluginsRegistryRefresh();
	const snapshot = await readConfigFileSnapshot();
	const cfg = snapshot.sourceConfig ?? snapshot.config;
	const report = buildPluginRegistrySnapshotReport({ config: cfg });
...
async function runPluginsInstallAction(raw, opts) {
	await tracePluginLifecyclePhaseAsync("install command", async () => {
		const { runPluginInstallCommand } = await import("./plugins-install-command-DaafkRxz.js");
...
async function runPluginsRegistryCommand(opts) {
	const { inspectPluginRegistry, refreshPluginRegistry } = await import("./plugin-registry-C64pDFi9.js");
...
async function runPluginMarketplaceListCommand(source, opts) {
	const { listMarketplacePlugins } = await import("./marketplace-su-DBK-z.js");
