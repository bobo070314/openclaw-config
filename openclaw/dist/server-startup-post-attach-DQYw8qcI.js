...
import { i as getPluginModuleLoaderStats } from "./plugin-module-loader-cache-B1R5eja6.js";
...
const loadMainSessionRestartRecoveryModule = async () => {
	mainSessionRestartRecoveryModulePromise ??= import("./main-session-restart-recovery-DuPwGiPI.js");
...
const loadAgentModelSelectionModule = async () => {
	agentModelSelectionModulePromise ??= import("./model-selection-BQVGL39u.js");
	return await agentModelSelectionModulePromise;
};
const loadInternalHooksModule = async () => {
	internalHooksModulePromise ??= import("./internal-hooks-DGoxu5Op.js");
	return await internalHooksModulePromise;
};
const loadGatewayRestartSentinelModule = async () => {
	gatewayRestartSentinelModulePromise ??= import("./server-restart-sentinel-CS0Rc-4S.js");
...
function scheduleGatewayMemoryBackend(params) {
	...
	const start = () => {
		import("./server-startup-memory-PAVtlVE3.js").then(({ startGatewayMemoryBackend }) => startGatewayMemoryBackend({
			cfg: params.cfg,
			log: params.log
		})).catch((err) => {
...
function scheduleProviderAuthStatePrewarm(params) {
	...
	(async () => {
		...
		const loadProviderAuthWarmModule = () => import("./model-provider-auth-CFDO1r_5.js");
...
function scheduleAgentRuntimePluginPrewarm(params) {
	...
	timer = setTimeout(() => {
		...
		measureStartup(params.startupTrace, "post-ready.agent-runtime-plugins", async () => {
			...
			const { ensureRuntimePluginsLoaded } = await import("./runtime-plugins-DgtnWIQO.js");
			const cfg = params.getConfig();
			if (isStopped()) return;
...
function scheduleTranscriptsAutoStartSidecar(params) {
	...
	return schedulePostReadySidecarTask({
		...
		log: params.log,
		run: async (isStopped) => {
			const { createTranscriptsAutoStartService } = await import("./transcripts-tool-B7jow2Sb.js");
			if (isStopped()) return;
...
async function waitForAcpRuntimeBackendReady(params) {
	const { getAcpRuntimeBackend } = await import("./registry-CLJxUptw.js");
...
async function prewarmConfiguredPrimaryModel(params) {
	const { resolveAgentModelPrimaryValue } = await import("./model-input-KcFi4CeL.js");
	...
	const [{ resolveAgentWorkspaceDir, resolveDefaultAgentDir, resolveDefaultAgentId }, { DEFAULT_MODEL, DEFAULT_PROVIDER }, { isCliProvider, resolveConfiguredModelRef }] = await Promise.all([
		import("./agent-scope-BxpxzPcu.js"),
	...
	const { ensureOpenClawModelsJson } = await import("./models-config-B8huWk6V.js");
...
async function startGatewaySidecars(params) {
	...
	await measureStartup(params.startupTrace, "sidecars.internal-hooks", async () => {
		try {
			if (internalHooksConfigured) {
				const [{ setInternalHooksEnabled }, { loadInternalHooks }] = await Promise.all([loadInternalHooksModule(), import("./loader-DItZyqhP.js")]);
	...
	let pluginServices = params.shouldStartPluginServices?.() === false ? null : await measureStartup(params.startupTrace, "sidecars.plugin-services", async () => {
		try {
			const { startPluginServices } = await import("./services-BVI1W54V.js");
	...
	schedulePostReadySidecarTask({
		...
		run: async (isStopped) => {
			try {
				const [{ resolveAgentSessionDirs }, { cleanStaleLockFiles }] = await Promise.all([import("./session-dirs-BUmx3Yg2.js"), import("./session-write-lock-ml9nEstl.js")]);
	...
	schedulePostReadySidecarTask({
		...
		log: params.log,
		run: async () => {
			const { scheduleSubagentOrphanRecovery } = await import("./subagent-registry-TWUgF0Gd.js");
	...
	if (params.cfg.hooks?.enabled && params.cfg.hooks.gmail?.account) postReadySidecars.push(schedulePostReadySidecarTask({
		...
		run: async (isStopped, signal) => {
			const { startGmailWatcherWithLogs } = await import("./gmail-watcher-lifecycle-BjEvb_Ca.js");
			if (isStopped()) return;
	...
	if (params.cfg.hooks?.gmail?.model) postReadySidecars.push(schedulePostReadySidecarTask({
		...
		run: async (isStopped) => {
			const [{ DEFAULT_MODEL, DEFAULT_PROVIDER }, { loadModelCatalog }, { getModelRefStatus, resolveConfiguredModelRef, resolveHooksGmailModel }] = await Promise.all([
				loadAgentDefaultsModule(),
				import("./model-catalog-BQZlAOIC.js"),
				loadAgentModelSelectionModule()
			]);
			if (isStopped()) return;
...
const defaultGatewayPostAttachRuntimeDeps = {
	...
	logGatewayStartup: async (params) => (await import("./server-startup-log-BqLcQ_At.js")).logGatewayStartup(params),
	refreshLatestUpdateRestartSentinel: refreshLatestUpdateRestartSentinelIfPresent,
	scheduleGatewayUpdateCheck: async (...args) => (await import("./update-startup-DB8Je5nF.js")).scheduleGatewayUpdateCheck(...args),
	startGatewaySidecars,
	startGatewayTailscaleExposure: async (...args) => (await import("./server-tailscale-czLz3Kpp.js")).startGatewayTailscaleExposure(...args)
};
...
async function startGatewayPostAttachRuntime(params, runtimeDeps = defaultGatewayPostAttachRuntimeDeps) {
	...
	sidecarsPromise.then(async (sidecarsResult) => {
		...
		if (hookRunner?.hasHooks("gateway_start")) {
			const { withPluginHttpRouteRegistry } = await import("./http-registry-CPhhp-Ny.js");
