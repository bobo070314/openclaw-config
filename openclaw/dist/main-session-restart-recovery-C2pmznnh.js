...
import { n as deliveryContextFromSession, o as normalizeDeliveryContext } from "./delivery-context.shared-BjCF5MAj.js";
import { i as resolveSessionFilePath, s as resolveSessionTranscriptPathInDir } from "./paths-BcGVZo_k.js";
import { t as loadSessionStore } from "./store-load-S40KNQ3r.js";
import { t as isDeliverableMessageChannel } from "./message-channel-normalize-B2zqLi2s.js";
...
import { r as applyRestartRecoveryLifecycle } from "./session-accessor-ArmkJJUW.js";
import "./sessions-BoPlPkQh.js";
import { d as listActiveEmbeddedRunSessionKeys, u as listActiveEmbeddedRunSessionIds } from "./run-state-B31lb2Ak.js";
		...
		const storeResult = await applyRestartRecoveryLifecycle({
			...
			update: (entries) => {
				...
				for (const { sessionKey, entry } of entries) {
					...
					const matchingActiveRuns = activeRuns.filter((run) => (run.sessionId ? run.sessionId === entry.sessionId : run.sessionKey === sessionKey) && (entry.status === "running" || run.observedAt === void 0 || normalizeFiniteTimestamp(entry.updatedAt) === void 0 || entry.updatedAt < run.observedAt) && params.isActiveRun?.(run) !== false);
