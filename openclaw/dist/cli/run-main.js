...
import { n as resolveManifestCommandAliasOwnerInRegistry, r as resolveManifestToolOwnerInRegistry } from "../manifest-command-aliases-BIZgFkJv.js";
import { t as ensureOpenClawCliOnPath } from "../path-env-CXeWaPjD.js";
import { a as shouldSkipPluginCommandRegistration, r as shouldRegisterPrimaryCommandOnly, t as isReservedNonPluginCommandRoot } from "../command-registration-policy-BVV9gO5J.js";
import { a as consumeGatewayFastPathRootOptionToken, c as resolveGatewayRunPreBootstrapOptions, n as resolveCliNetworkProxyPolicy, o as consumeGatewayRunOptionToken, s as resolveGatewayCatalogCommandPath, t as resolveCliCommandPathPolicy } from "../command-path-policy-Cc8RHm5e.js";
import { n as withConsoleLogsRoutedToStderrForJson, t as hasJsonOutputFlag } from "../json-output-mode-uwhtsLhD.js";
import { t as formatCliCommandSuggestions } from "../command-suggestions-DHuVfX1C.js";
...
const loadRootHelpMetadataModule = async () => await import("../root-help-metadata-DbX7xxFH.js");
const loadLoggingModule = async () => await import("../logging-vf6EORl6.js");
const loadCliRegistryLoaderModule = async () => await import("../cli-registry-loader-DJXK3ItO.js");
const loadManifestCommandAliasesRuntimeModule = async () => await import("../manifest-command-aliases.runtime-BtPrAUxi.js");
const loadProxyLifecycleModule = async () => await import("../proxy-lifecycle-FqW5UC_I.js");
...
async function tryRunGatewayRunFastPath(argv, startupTrace) {
	...
	const [{ Command }, { addGatewayRunCommand }, { VERSION }, { emitCliBanner }, { resolveCliStartupPolicy }, { enableConsoleCapture }, { ensureCliExecutionBootstrap }, { defaultRuntime }] = await startupTrace.measure("gateway-run-imports", () => Promise.all([
		import("commander"),
		import("../run-command-CH0zuZdH.js"),
		import("../version-BmsGkjsI.js"),
		import("../banner-DvPyAddf.js"),
		import("../command-startup-policy-C_A5dfNm.js"),
		loadLoggingModule(),
		import("../command-execution-startup-BmmxLPYK.js"),
	...
	const beforeRun = async (opts) => {
		...
		if (!await startupTrace.measure("gateway-run-pre-bootstrap", async () => {
			const { prepareGatewayRunBootstrap, recheckGatewayRunBootstrap } = await import("../pre-bootstrap-CbqVKvpF.js");
		...
		await startupTrace.measure("gateway-run-bootstrap", async () => {
			...
			const { reloadTrustedGatewayRunEnvironment } = await import("../pre-bootstrap-CbqVKvpF.js");
...
async function closeCliMemoryManagers() {
	try {
		...
		const { closeActiveMemorySearchManagers } = await import("../memory-runtime-CO_xTtco.js");
		await closeActiveMemorySearchManagers();
	} catch {}
}
async function disposeCliAgentHarnesses() {
	try {
		const { listRegisteredAgentHarnesses, disposeRegisteredAgentHarnesses } = await import("../registry-DbQmNC8_.js");
...
async function bootstrapCliProxyCaptureAndDispatcher(startupTrace, options = {}) {
	const [{ initializeDebugProxyCapture, finalizeDebugProxyCapture }, { maybeWarnAboutDebugProxyCoverage }] = await startupTrace.measure("proxy-imports", () => Promise.all([import("../runtime-B8vNArbK.js"), import("../coverage-DA_faJak.js")]));
...
async function runCli(argv = process$1.argv) {
	...
	if (!isHelpOrVersionInvocation && (isGatewayRunInvocation || shouldLoadCliDotEnv())) await startupTrace.measure("dotenv", async () => {
		if (isRemoteAgentDispatchInvocation(normalizedArgv, normalizedInvocation.primary)) {
			const { loadGatewayDispatchCliDotEnv } = await import("../gateway-dispatch-dotenv-BzALykYf.js");
			await loadGatewayDispatchCliDotEnv({ quiet: true });
		} else {
			const { loadCliDotEnv } = await import("../dotenv-DAyqZf2o.js");
			loadCliDotEnv({
	...
	if (!isHelpOrVersionInvocation && isGatewayRunInvocation) await startupTrace.measure("gateway-run-select-environment", async () => {
		const [{ selectGatewayRunEnvironment }, { defaultRuntime }] = await Promise.all([import("../pre-bootstrap-CbqVKvpF.js"), import("../runtime-Dc6wmxIA.js")]);
	...
	const readBestEffortCliConfig = async () => {
		if (!bestEffortConfigPromise) bestEffortConfigPromise = import("../io-t6re4Xxv.js").then(({ readBestEffortConfig }) => readBestEffortConfig(isolateProxyConfigEnv ? {
			isolateEnv: true,
			observe: false
		} : void 0));
	...
	try {
		if (shouldUseRootHelpFastPath(normalizedArgv)) {
			...
			const { outputRootHelp } = await import("../root-help-CZB9iR6F.js");
		...
		if (shouldUseSetupOnboardConfigureHelpFastPath(normalizedArgv)) {
			const { tryOutputSetupOnboardConfigureHelp } = await import("../setup-onboard-configure-help-fast-path-BJ0U_bDI.js");
		...
		if (shouldRunBareRootCrestodian) {
			if (await shouldStartOnboardingForFreshInstall(normalizedArgv)) {
				...
				const { setupWizardCommand } = await import("../onboard-CfCdWYIz.js");
		...
		if (bootstrapProxyBeforeFastPath && await tryRunGatewayRunFastPath(normalizedArgv, startupTrace)) return;
		const { tryRouteCli } = await startupTrace.measure("route-import", () => import("../route-DbzxogG3.js"));
		...
		try {
			...
			const [{ buildProgram }, { formatUncaughtError }, { formatCliFailureLines }, { runFatalErrorHooks }, { installUnhandledRejectionHandler, isBenignUncaughtExceptionError, isUncaughtExceptionHandled }, { restoreTerminalState }] = await startupTrace.measure("core-imports", () => Promise.all([
				import("../program-D2vmsZ5J.js"),
				import("../infra/errors.js"),
				import("../failure-output-2W8vvj5x.js"),
				import("../fatal-error-hooks-BiXaGF_Q.js"),
				import("../unhandled-rejections-Bp1YqXfP.js"),
			...
			if (primary && shouldRegisterPrimaryCommandOnly(parseArgv)) await startupTrace.measure("register-primary", async () => {
				...
				if (ctx) {
					const { registerCoreCliByName } = await import("../command-registry--B0C5wNe.js");
					await registerCoreCliByName(program, ctx, primary, parseArgv);
				}
				const { registerSubCliByName } = await import("../register.subclis-Di34Kole.js");
			...
			if (!shouldSkipPluginCommandRegistration({
				...
				const config = await startupTrace.measure("register-plugin-commands", async () => {
					const { registerPluginCliCommandsFromValidatedConfig } = await import("../cli-tnJ1YkCQ.js");
