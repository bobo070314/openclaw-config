...
import { n as isCanvasHostEnabled, t as canvasConfigSchema } from "../../config-Dz7Zr0Zk.js";
import { n as CANVAS_HOST_PATH, r as CANVAS_WS_PATH, t as A2UI_PATH } from "../../a2ui-shared-BjqQ2tdx.js";
import { t as CanvasToolSchema } from "../../tool-schema-CzfkRAOx.js";
...
function createLazyCanvasTool(params) {
	...
	const loadTool = async () => {
		toolPromise ??= import("../../tool-2JSY5AmK.js").then(({ createCanvasTool }) => createCanvasTool({
	...
	register(api) {
		if (isCanvasHostEnabled(api.config)) {
			...
			const loadHttpRouteHandler = async () => {
				httpRouteHandlerPromise ??= import("../../http-route-Y3Omb4KG.js").then(({ createCanvasHttpRouteHandler }) => createCanvasHttpRouteHandler({
					config: api.config,
			...
			api.registerHostedMediaResolver(async (mediaUrl) => {
				resolveCanvasHttpPathToLocalPathPromise ??= import("../../documents-ChtyIRDg.js").then(({ resolveCanvasHttpPathToLocalPath }) => resolveCanvasHttpPathToLocalPath);
		...
		api.registerNodeCliFeature(async ({ program }) => {
			const { createDefaultCanvasCliDependencies, registerNodesCanvasCommands } = await import("../../cli-3rVJw0kX.js");
