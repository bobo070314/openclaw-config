...
import { r as hasAmbiguousGatewayAuthModeConfig } from "./auth-mode-policy-D0BbSs8Y.js";
import { t as resolveGatewayAuthToken } from "./auth-token-resolution-BzBeJlB1.js";
import "./auth-BDsIZbhK.js";
import { n as resolveGatewayAuth } from "./auth-resolve-S1BMuCdt.js";
import { n as maybeArchiveLegacyClawdBrowserProfileResidue, r as noteChromeMcpBrowserReadiness, t as detectLegacyClawdBrowserProfileResidue } from "./doctor-browser-gbanLs7h.js";
import { n as hasConfiguredCommandOwners } from "./doctor-command-owner-BCuWFLcz.js";
import { a as shellCompletionStatusToRepairEffects, i as shellCompletionStatusToHealthFindings, t as checkShellCompletionStatus } from "./doctor-completion-Dh7Q2Moy.js";
import { n as disableUnavailableSkillsInConfig, r as formatMissingSkillSummary } from "./doctor-skills-core-DZQVCoNO.js";
import { i as uiProtocolFreshnessIssueToRepairEffects, r as uiProtocolFreshnessIssueToHealthFinding, t as detectUiProtocolFreshnessIssues } from "./doctor-ui-Baaq3tCg.js";
import { n as collectDisabledCodexPluginRouteIssues } from "./codex-route-warnings-BpGlpLc4.js";
import { t as getSkippedExecRefStaticError } from "./exec-resolution-policy-Dydzl4UT.js";
...
const loadDoctorWorkspaceModule = async () => await import("./doctor-workspace-wmLOQPN4.js");
...
async function collectSecurityWarningsWithRuntime(cfg) {
	const { collectSecurityWarnings } = await import("./doctor-security-CH9TQG5e.js");
	return collectSecurityWarnings(cfg);
}
async function collectWorkspaceSuggestionNotesWithRuntime(workspaceDir) {
	const { collectWorkspaceBackupTip } = await import("./doctor-state-integrity-BbdG0V0k.js");
...
const hooksModelCheck = {
	...
	async detect(ctx) {
		...
		const { loadModelCatalog } = await import("./model-catalog-BQZlAOIC.js");
		const { getModelRefStatus, resolveConfiguredModelRef, resolveHooksGmailModel } = await import("./model-selection-BQVGL39u.js");
...
const legacyStateCheck = {
	...
	source: "doctor",
	async detect(ctx) {
		const { detectLegacyStateMigrations } = await import("./doctor-state-migrations-BwmU2HXc.js");
...
const bootstrapSizeCheck = {
	...
	async detect(ctx) {
		...
		const { resolveBootstrapContextForRun } = await import("./bootstrap-files-B4E27bZF.js");
		const { resolveBootstrapMaxChars, resolveBootstrapTotalMaxChars } = await import("./embedded-agent-helpers-CjLluTym.js");
...
const openAIOAuthTlsCheck = {
	...
	source: "doctor",
	async detect(ctx) {
		const { formatOpenAIOAuthTlsPreflightFix, runOpenAIOAuthTlsPreflight, shouldRunOpenAIOAuthTlsPrerequisites } = await import("./provider-openai-chatgpt-oauth-tls-Cjkl6xAv.js");
...
const legacyWhatsAppCrontabCheck = {
	...
	source: "doctor",
	async detect() {
		const { collectLegacyWhatsAppCrontabHealthWarning } = await import("./cron-BPwqOmBa.js");
...
const gatewayPlatformNotesCheck = {
	...
	source: "doctor",
	async detect(ctx) {
		const { collectMacGatewayPlatformWarnings } = await import("./doctor-platform-notes-A-Md97fz.js");
