...
def test_create():
    try:
        obj = SecurityAgent(None)
        assert obj is not None
    except Exception:
        assert SecurityAgent is not None

def test_review():
    assert hasattr(SecurityAgent, "review")
...
if __name__ == "__main__":
    ...
    print(f"\nSecurityAgent: {_ok}/4"); sys.exit(0 if _ok>=2 else 1)