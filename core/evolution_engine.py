...
def load_state():
    if STATE_FILE.exists():
        try:
            ...
            pass
    base = {
        "weights": {"deploy": 0.9, "high_traffic": 0.5, "config_change": 0.7,
                    "build_process": 0.6, "disk_write": 0.8, "memory_leak": 0.7,
                    "git_push": 0.8, "cron_job": 0.4},
...
def ask_deepseek_for_weight_adjustment(state, wrong_cause, real_cause):
    ...
    try:
        ...
        client = OpenAI(api_key=os.environ.get("DEEPSEEK_API_KEY", os.environ.get("OPENAI_API_KEY", "")),
        ...
        return {"reason": f"Mechanical fallback: {wrong_cause}->{real_cause}",
...
def apply_deepseek_suggestion(state, suggestion):
    ...
    state.setdefault("deepseek_history", []).append({
        "ts": now, "reason": reason, "applied": applied,
    })
...
def inject_weights_into_reasoner(state):
    ...
    subprocess.run(["git", "-C", str(SKILLS), "add", "causal-reasoner/run.py"], check=False,
                   encoding="utf-8", errors="replace")
    res = subprocess.run(
        ["git", "-C", str(SKILLS), "commit", "-m",
...
def detect_alerts():
    ...
    if disk_usage > 90:
        alerts.append({"type": "disk", "severity": "high",
...
def task_agent(agent_id, args_str, alert):
    ...
    cmd = [sys.executable, str(script), "--dry-run", args_str.split()[0]] if "--dry-run" not in sys.argv else \
    ...
    if daemon_state.exists():
        try:
            ...
            if total_alerts >= 3:
                ...
                log_cycle(state, {"action": "escalate", "alerts": total_alerts,
...
def rollback():
    ...
    script = AGENTS["ops"]
    subprocess.run([sys.executable, str(script), "--rollback"],
