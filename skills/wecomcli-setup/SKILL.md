...
## Overview
企业微信CLI环境配置 - 安装wecomcli、配置corpid/secret、验证连接、环境诊断

## Category
enterprise

## When to Use
Trigger this skill when the user requests operations related to 企业微信setup.

## Workflow

### Pre-flight
1. Verify prerequisites are met (dependencies, credentials, environment)
2. Check configuration via `wecomcli-setup-setup` or equivalent if available
...
```
User: 用 wecomcli-setup 执行基本操作
Assistant: [调用 exec 执行相应命令，返回格式化结果]
```

### Example 2: Error Handling
```
User: 用 wecomcli-setup 操作一个不存在的资源
