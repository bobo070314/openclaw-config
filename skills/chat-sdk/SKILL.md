...
## Overview
聊天SDK集成 - 统一消息接口，支持多平台消息收发（企微/飞书/钉钉）和会话管理

## Category
devsecops

## When to Use
Trigger this skill when the user requests operations related to chat sdk.

## Workflow

### Pre-flight
1. Verify prerequisites are met (dependencies, credentials, environment)
2. Check configuration via `chat-sdk-setup` or equivalent if available
...
```
User: 用 chat-sdk 执行基本操作
Assistant: [调用 exec 执行相应命令，返回格式化结果]
```

### Example 2: Error Handling
```
User: 用 chat-sdk 操作一个不存在的资源
