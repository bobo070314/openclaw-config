...
## Overview
飞书(Lark)应用配置 - 创建飞书应用、配置权限、获取凭证、Webhook 设置

## Category
enterprise

## When to Use
Trigger this skill when the user requests operations related to lark setup.

## Workflow

### Pre-flight
1. Verify prerequisites are met (dependencies, credentials, environment)
2. Check configuration via `lark-setup-setup` or equivalent if available
...
```
User: 用 lark-setup 执行基本操作
Assistant: [调用 exec 执行相应命令，返回格式化结果]
```

### Example 2: Error Handling
```
User: 用 lark-setup 操作一个不存在的资源
