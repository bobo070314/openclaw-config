...
## Overview
企业微信会议管理 - 创建/取消/查询会议，管理参会人员和会议录制

## Category
enterprise

## When to Use
Trigger this skill when the user requests operations related to 企业微信meeting.

## Workflow

### Pre-flight
1. Verify prerequisites are met (dependencies, credentials, environment)
2. Check configuration via `wecomcli-meeting-setup` or equivalent if available
...
```
User: 用 wecomcli-meeting 执行基本操作
Assistant: [调用 exec 执行相应命令，返回格式化结果]
```

### Example 2: Error Handling
```
User: 用 wecomcli-meeting 操作一个不存在的资源
