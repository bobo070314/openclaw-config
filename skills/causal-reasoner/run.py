...
AUTO_WEIGHTS = {"deploy": 0.9, "high_traffic": 0.5, "config_change": 0.7, "build_process": 0.6, "disk_write": 0.8, "memory_leak": 0.7, "git_push": 0.8, "cron_job": 0.4}
# === END AUTO-TUNE ===
    ...
    if evidence_path and os.path.exists(evidence_path):
        try:
            ...
            for key in ["deploy", "git_push", "config_change", "high_traffic",
                         "disk_write", "memory_leak", "cron_job", "build_process"]:
                if state.get(key):
...
def main():
    ...
    if args.json:
        ...
        print(f"  Top cause: {causes[0]['cause']} ({causes[0]['confidence']:.0%})" if causes else "  Top cause: unknown")
