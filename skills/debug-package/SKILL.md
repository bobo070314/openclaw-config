...
## Overview
Node.js 包调试工具 - 快速定位 node_modules 问题，符号链接修复、版本冲突检测

## Category
devsecops

## When to Use
Trigger this skill when the user requests operations related to debug package.

## Workflow

### Pre-flight
1. Verify prerequisites are met (dependencies, credentials, environment)
2. Check configuration via `debug-package-setup` or equivalent if available
...
```
User: 用 debug-package 执行基本操作
Assistant: [调用 exec 执行相应命令，返回格式化结果]
```

### Example 2: Error Handling
```
User: 用 debug-package 操作一个不存在的资源
