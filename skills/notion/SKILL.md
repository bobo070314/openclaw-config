...
## Overview
Notion 工作区集成 - 页面创建/查询/更新，数据库操作，Block 级别内容管理

## Category
productivity

## When to Use
Trigger this skill when the user requests operations related to notion.

## Workflow

### Pre-flight
1. Verify prerequisites are met (dependencies, credentials, environment)
2. Check configuration via `notion-setup` or equivalent if available
...
```
User: 用 notion 执行基本操作
Assistant: [调用 exec 执行相应命令，返回格式化结果]
```

### Example 2: Error Handling
```
User: 用 notion 操作一个不存在的资源
