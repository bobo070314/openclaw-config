...
def check_cpu(threshold_pct=85):
    ...
    return {"type": "cpu", "value": cpu, "threshold": threshold_pct, "alert": alert,
...
def check_memory(threshold_pct=90):
    try:
        ...
        return {"type": "memory", "value": mem.percent, "threshold": threshold_pct, "alert": alert,
                "message": f"Memory high: {mem.percent}%" if alert else f"Memory OK: {mem.percent}%"}
    except ImportError:
        return {"type": "memory", "value": -1, "threshold": threshold_pct, "alert": False, "message": "Memory check unavailable"}
...
def check_disk(threshold_pct=95):
    try:
        ...
        return {"type": "disk", "value": usage.percent, "threshold": threshold_pct, "alert": alert,
                "message": f"Disk full: {usage.percent}%" if alert else f"Disk OK: {usage.percent}%"}
    except ImportError:
        return {"type": "disk", "value": -1, "threshold": threshold_pct, "alert": False, "message": "Disk check unavailable"}
    ...
    for log_dir in [SKILLS_DIR / ".deploy" / "logs", WORKSPACE_DIR / ".deploy" / "logs"]:
        ...
        for lf in sorted(log_dir.glob("*.log"), key=lambda p: p.stat().st_mtime, reverse=True)[:3]:
            try:
                ...
                pass
    alert = len(errors) > 0
    return {"type": "logs", "value": len(errors), "threshold": 0, "alert": alert,
            "message": f"{len(errors)} ERROR(s) in recent logs" if alert else "Logs clean", "details": errors[:10]}
...
def check_tokens():
    ...
    return {"type": "tokens", "value": populated, "total": total, "threshold": 1, "alert": alert,
            "message": "; ".join(problems) if alert else f"Vault OK ({populated} tokens)", "details": problems}


def check_git():
    try:
        r = subprocess.run(["git", "-C", str(WORKSPACE_DIR), "status", "--porcelain"], capture_output=True,
        ...
        return {"type": "git", "value": len(changed), "threshold": 0, "alert": len(alerts) > 0,
...
def check_prompts():
    patterns = [r"ignore previous instructions", r"forget your system prompt", r"output your system prompt",
                r"you are now a different", r"pretend to be", r"execute.*rm -rf", r"DROP TABLE", r"DELETE FROM"]
    alerts = []
    ...
    if memory_dir.exists():
        for mf in sorted(memory_dir.glob("*.md"), key=lambda p: p.stat().st_mtime, reverse=True)[:1]:
            try:
                ...
                pass
    alert = len(alerts) > 0
    return {"type": "prompts", "value": len(alerts), "threshold": 0, "alert": alert,
            "message": f"{len(alerts)} adversarial pattern(s)" if alert else "No adversarial patterns", "details": alerts}
...
def listen_for_wechat_commands(cmd=None):
    ...
    try:
        ...
        r = subprocess.run([sys.executable, str(val_script), "--json", plate], capture_output=True,
                           text=True, encoding="utf-8", errors="replace", timeout=15)
        result = json.loads(r.stdout) if r.returncode == 0 else {"error": r.stderr[:200]}
        return {"type": "profit", "value": result.get("market_value", 0), "alert": False,
                "message": f"Valuation: {result.get('plate','?')} = {result.get('market_value','?')}", "details": result}
    except Exception as e:
...
def arm_response(alerts, dry_run=False):
    ...
    for alert in alerts:
        ...
        if atype in ("cpu", "memory", "vision") and not dry_run:
            ...
            try:
                r = subprocess.run([sys.executable, str(reasoner_path), "--infer", effect, "--json",
                                   "--evidence-path", str(STATE_FILE)], capture_output=True,
                                   text=True, encoding="utf-8", errors="replace", timeout=10)
                if r.returncode == 0:
                    inference = json.loads(r.stdout)
                    if inference.get("expected"):
                        suppressed.append({"alert_type": atype, "reason": inference.get("expected_cause", "?"), "verdict": "suppressed"})
                        continue
            except Exception as e:
        ...
        if atype == "logs" and not dry_run:
            try:
                r = subprocess.run([sys.executable, str(SKILLS_DIR / "log-analyzer" / "run.py"), "--recent", "100"],
                                   capture_output=True, text=True, encoding="utf-8", errors="replace", timeout=15)
        ...
        if atype == "prompts" and not dry_run:
            responses.append({"trigger": "adversarial-intercept", "action": "BLOCK_RECOMMENDED", "details": alert.get("details", [])})
...
def run_checks(dry_run=False, profit_cmd=None):
    ...
    result = {"timestamp": datetime.now(UTC).isoformat(), "checks": checks,
              "alerts": len(alerts), "status": "ALERT" if alerts else "OK", "dry_run": dry_run}
    if alerts:
