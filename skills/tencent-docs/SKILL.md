...
## Overview
腾讯文档集成 - 创建/编辑/导出在线文档、表格、幻灯片，支持协作和权限管理

## Category
productivity

## When to Use
Trigger this skill when the user requests operations related to tencent docs.

## Workflow

### Pre-flight
1. Verify prerequisites are met (dependencies, credentials, environment)
2. Check configuration via `tencent-docs-setup` or equivalent if available
...
```
User: 用 tencent-docs 执行基本操作
Assistant: [调用 exec 执行相应命令，返回格式化结果]
```

### Example 2: Error Handling
```
User: 用 tencent-docs 操作一个不存在的资源
...
- Check category peers under `productivity`
