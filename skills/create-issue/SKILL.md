...
## Overview
GitHub Issue 创建 - 结构化Issue模板、自动标签、项目看板关联和通知

## Category
devsecops

## When to Use
Trigger this skill when the user requests operations related to create issue.

## Workflow

### Pre-flight
1. Verify prerequisites are met (dependencies, credentials, environment)
2. Check configuration via `create-issue-setup` or equivalent if available
...
```
User: 用 create-issue 执行基本操作
Assistant: [调用 exec 执行相应命令，返回格式化结果]
```

### Example 2: Error Handling
```
User: 用 create-issue 操作一个不存在的资源
