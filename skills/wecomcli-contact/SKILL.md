...
## Overview
企业微信通讯录管理 - 查询、添加、更新、删除企业微信联系人，支持部门遍历和批量导入

## Category
enterprise

## When to Use
Trigger this skill when the user requests operations related to 企业微信contact.

## Workflow

### Pre-flight
1. Verify prerequisites are met (dependencies, credentials, environment)
2. Check configuration via `wecomcli-contact-setup` or equivalent if available
...
```
User: 用 wecomcli-contact 执行基本操作
Assistant: [调用 exec 执行相应命令，返回格式化结果]
```

### Example 2: Error Handling
```
User: 用 wecomcli-contact 操作一个不存在的资源
