...
## Overview
企业微信文档操作 - 创建/编辑/删除在线文档和表格，支持权限管理和协作编辑

## Category
enterprise

## When to Use
Trigger this skill when the user requests operations related to 企业微信doc.

## Workflow

### Pre-flight
1. Verify prerequisites are met (dependencies, credentials, environment)
2. Check configuration via `wecomcli-doc-setup` or equivalent if available
...
```
User: 用 wecomcli-doc 执行基本操作
Assistant: [调用 exec 执行相应命令，返回格式化结果]
```

### Example 2: Error Handling
```
User: 用 wecomcli-doc 操作一个不存在的资源
