...
## Overview
企业微信日程管理 - 创建/查询/更新/取消日程，支持重复日程和提醒设置

## Category
enterprise

## When to Use
Trigger this skill when the user requests operations related to 企业微信schedule.

## Workflow

### Pre-flight
1. Verify prerequisites are met (dependencies, credentials, environment)
2. Check configuration via `wecomcli-schedule-setup` or equivalent if available
...
```
User: 用 wecomcli-schedule 执行基本操作
Assistant: [调用 exec 执行相应命令，返回格式化结果]
```

### Example 2: Error Handling
```
User: 用 wecomcli-schedule 操作一个不存在的资源
