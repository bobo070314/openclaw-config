...
One skill for all agentic end-to-end testing 鈥?local-first today, designed to
also run as full cloud automation. Every test session follows the same
four-step contract:

```
Step -1: Plan approval  鈫? Step 0: Env + Auth  鈫? Step 1: Pick surface  鈫? Step 2: Run  鈫? Step 3: Structured report
```

## Step -1 鈥?Plan approval for non-trivial tests
...
1. `寮€濮嬫墽琛?(Recommended)` 鈥?娴嬭瘯鏂规娌￠棶棰橈紝寮€濮嬫墽琛?2. `鍏堣璁轰笅` 鈥?鏂规鏈夐棶棰橈紝鍏堣璁轰笅

Wait for the user's choice before proceeding.

## Step 0 鈥?Environment setup + auth check (mandatory)
...
a login wall wastes the whole session 鈥?clear both gates BEFORE writing a
single test step.
...
### 0.1 Dependencies are installed 鈥?root AND standalone apps

The root pnpm workspace does **NOT** cover every app: `pnpm-workspace.yaml`
lists `packages/**`, `e2e`, `apps/server`, and only `apps/desktop/src/main` 鈥?**`apps/desktop` and `apps/cli` are standalone**, each keeping its own
...
recently added workspace package 鈥?`Rolldown failed to resolve import
...
directory 鈥?a script launched while `cwd` is `apps/desktop` fails with
...
Use `scripts/init-dev-env.sh`. It follows the e2e setup pattern 鈥?Postgres,
Redis, migrations, auth/key-vault/S3 test env, seed user 鈥?but it is owned by this
...
| Bot      | Native apps already logged in                 | 鈥?                       | per-platform screenshot                   |

Login-state checks are standardized 鈥?do NOT hand-roll `window.__LOBE_STORES`
...
## Step 1 鈥?Pick the surface by change scope
...
| **Bot channels** (Discord / WeChat / Lark / 鈥?          | Native app via osascript / bridge    | Only way to exercise the real channel end-to-end                  | `bot/<platform>/index.md`          |
...
| -------- | ------------- | --------------------------------------------------------- | ------------------------------------------------------ |
| CLI      | 鉁?           | 鉁?                                                       | n/a 鈥?text output                                      |
| Web      | 鉁?           | 鉁?headless Chromium works natively                       | CDP 鈥?no display needed                                |
| Electron | 鉁?           | 鈿狅笍 runs, but needs a display server: wrap with `xvfb-run` | CDP works under Xvfb; `capture-app-window.sh` does NOT |
| Bot      | 鉁?           | 鉂?osascript + native apps are macOS-only                 | macOS `screencapture` only                             |
...
| WeChat / 寰俊 | [bot/wechat/index.md](./bot/wechat/index.md)     | `Cmd+F`               |
| Lark / 椋炰功   | [bot/lark/index.md](./bot/lark/index.md)         | `Cmd+K`               |
...
[references/osascript.md](./references/osascript.md) first 鈥?it is a general
...
| `record-gif.sh`           | Frame-sequence 鈫?GIF for time-based behavior (streaming, timers, animations) |
...
`app-probe.sh` is the LobeHub-specific fast path into app state 鈥?auth check,
current route, running operations, and `goto <path>` quick navigation
(`/agent/<agentId>/<topicId>`, `/task/<taskId>`, `/settings`, 鈥? so a test can
...
## Step 3 鈥?Structured report (mandatory deliverable)

Every automated test session ends with a structured, evidence-backed report 鈥?not a chat-only summary. Scaffold it up front and fill it as you test:
...
- **Report language = the user's conversation language.** Write the ENTIRE
  `report.md` (headings included) in the language the user is conversing in 鈥?  no mixed English. `result.json` keys/status values stay English.
...
- **Time-based behavior needs a GIF, not a screenshot.** If a case asserts
  ...
  animations), record it with `scripts/record-gif.sh` and embed the GIF 鈥?  a static screenshot cannot prove the behavior.

## Directory map

```
agent-testing/
鈹溾攢鈹€ SKILL.md            # this router
鈹溾攢鈹€ cli/index.md        # backend verification via the LobeHub CLI
鈹溾攢鈹€ ui/electron.md      # pure-frontend verification in the desktop app
鈹溾攢鈹€ ui/web.md           # full-stack verification in the browser
鈹溾攢鈹€ bot/<platform>/     # bot-channel verification (osascript / bridge)
鈹溾攢鈹€ references/         # shared knowledge: auth, dev-server, agent-browser, osascript, report
鈹斺攢鈹€ scripts/            # setup-auth, report-init, electron-dev, capture, recording, gateway
```
