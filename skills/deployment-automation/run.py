...
def safe_run(cmd, cwd=None, timeout=30):
    result = subprocess.run(cmd, capture_output=True, text=True, encoding="utf-8", errors="replace", cwd=cwd, timeout=timeout)
...
def main():
    ...
    if args.rollback:
        print(json.dumps({"action": "rollback", "status": "rolling_back",
                          "note": "Restoring last stable version"}, indent=2))
        if not args.dry_run:
    ...
    if args.json and not args.action:
        info = {"skill": SKILL_NAME, "version": VERSION, "status": "live", "features": ["deploy", "rollback", "health"], "requires": ["docker", "kubectl (optional)"]}
