...
## Overview
内置工具注册与管理 - 动态注册/注销工具函数，工具发现、参数校验和调用日志

## Category
devsecops

## When to Use
Trigger this skill when the user requests operations related to builtin tool.

## Workflow

### Pre-flight
1. Verify prerequisites are met (dependencies, credentials, environment)
2. Check configuration via `builtin-tool-setup` or equivalent if available
...
```
User: 用 builtin-tool 执行基本操作
Assistant: [调用 exec 执行相应命令，返回格式化结果]
```

### Example 2: Error Handling
```
User: 用 builtin-tool 操作一个不存在的资源
